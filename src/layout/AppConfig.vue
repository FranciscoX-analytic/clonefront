...
export default {
  ...
  data() {
    return {
      active: false,
      d_layoutMode: localStorage.getItem('layoutMode'),
  ...
  watch: {
    ...
    d_layoutMode: {
      handler(newLayoutMode) {
        this.applyLayout(newLayoutMode);
        localStorage.setItem('layoutMode', newLayoutMode);
      },
      immediate: true,
    },
    '$primevue.config.inputStyle': {
      handler(newInputStyle) {
        this.$primevue.config.inputStyle = newInputStyle;
        localStorage.setItem('inputStyle', newInputStyle);
      },
      immediate: true,
    },
     storedTheme: {
  ...
  mounted() {
    ...
    const storedInputStyle = localStorage.getItem('inputStyle');
    const storedTheme = localStorage.getItem('theme')? localStorage.getItem('theme'): 'bootstrap4-dark-blue';
    const storedDark = localStorage.getItem('dark')== 'true' ? true : false;

    
    console.log("Initial newStored:", storedTheme)
    console.log("Stored theme:", localStorage.getItem('theme'));
    
    if (storedScale) {
      this.scale = parseInt(storedScale, 10); 
    }

    if (storedLayoutMode) {
      this.d_layoutMode = storedLayoutMode;
      this.applyLayout(storedLayoutMode);
    }

    if (storedInputStyle) {
      this.$primevue.config.inputStyle = storedInputStyle;
    }

    if (storedTheme) {
      EventBus.emit('theme-change', { theme: storedTheme, dark: storedDark });
      this.theme = storedTheme;
      this.applyTheme(storedTheme);
    }

    // if (storedDark) {
    //   this.dark = storedDark;
    // }
  ...
  computed: {
    ...
    inputStyle() {
      return localStorage.getItem('inputStyle');
    },
  },
};
</script>